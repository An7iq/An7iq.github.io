...
export default async function ProjectPage({
  ...
  const approachTitle = study.approachTitle ?? "Analytical approach";
  const findingsTitle = study.findingsTitle ?? "Key findings or current evidence";
  const showOverview =
    !study.skipOverview &&
      ...
      <Container className="max-w-4xl">
        ...
        <div className="mt-10 space-y-2 rounded-2xl border border-sand bg-white p-6 shadow-[0_10px_28px_rgba(27,36,32,0.08)] sm:p-8">
          ...
          {figuresAfter("Research context").length ? (
            <ProjectFigures figures={figuresAfter("Research context")} />
          ) : null}
          {study.data ? (
            <Section title="Data">
